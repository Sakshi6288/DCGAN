...
## Initializing weights of neural network modules..
def weights_init(m):
...
## Creates an output directory for storing experimental results & model checkpoints..
def make_output_dir():
    root_path = '../output'
    args = sys.argv[1:]
    if len(args)==0:
...
## This function is designed to convert child class codes to parent class codes..
...
## This function is to save image results, including real & fake images to the directory..
...
## This function calculates the gradient penalty loss, which is used to enforce the lipschitz constraint on the discriminator by penalizing the gradient norm of the discriminator's output with respect to the input data points..
