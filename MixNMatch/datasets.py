...
## This code prepares image data for further processing by cropping regions of interest and resizing images..
def get_imgs(img_path, imsize, bbox=None, transform=None, normalize=None):
    ...
    if bbox is not None:
        ...
        # Adding this line below to check which bbox is creating error.
        #if x2 <= x1 or y2 <= y1:
    ...
    my_crop_width = 126
    re_fimg = transforms.Resize(int(my_crop_width * 80 / 64))(fimg)
...
class Dataset(data.Dataset):
    ## Initializes varoius attributes & loads necessary data..
    ...
    def prepair_training_pairs(self, index):
        ...
        ## Calling get_imgs function to obtain fimgs, cimgs & warped_bbox..
        fimgs, cimgs, warped_bbox = get_imgs(img_name, self.imsize,
                                             bbox, self.transform, normalize=self.norm)


        # Randomly generating child code during training
        # It randomly selects a class index from the range of fine-grained categories & creates on hot-encoded tensor for the selected class..
...
def get_dataloader(bs=None):
    ...
    image_transform = transforms.Compose([ transforms.Resize(int(imsize * 80 / 64)),
