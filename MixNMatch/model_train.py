...
## For processing feature maps with the same spatial resolution..
...
## Enable easier training by mitigating the vanishing gradient problem..
...
## These functions are defined to get the images and masks..
## 3x3 conv layer will convert the input feature maps to 3 channels image & Tanh (activation function) maps the output to the range [-1,1]..
class GET_IMAGE(nn.Module):
    def __init__(self, ngf):
        super().__init__()
        self.img = nn.Sequential( conv3x3(ngf, 3),  nn.Tanh() )
    ## Forward method will return the generated image.. 
    def forward(self, h_code):
        return self.img(h_code)

## Responsible function for generating masks..
## 3x3 conv layer will convert the input feature maps to 1 channel mask & sigmoid (activation function) maps the output to the range [0,1]..
class GET_MASK(nn.Module):
    def __init__(self, ngf):
        super().__init__()
        self.img = nn.Sequential( conv3x3(ngf, 1), nn.Sigmoid() )
    ## forward method returns the generated masks..   
    def forward(self, h_code):
        return self.img(h_code)

## This function is responsible for generating background image..
...
class BACKGROUND_STAGE(nn.Module):
    def __init__(self, ngf):
        ...
        self.ngf = ngf 
        ## concatenation of a noise vector and additional input vector..
        ...
        # 64*128*128
## Here each block is increasing the spatial resolution of the input feature map by upsampling..
## Final output of this block is the generated background image, represented as a tensor..
...
## It takes input codes & generates a feature map representing higher-level features of the generated content..
...
## Based on the input codes it will generate a child-stage feature map..
...
class G_NET(nn.Module):
    ...
    def forward(self, z_code, c_code, p_code, b_code, which, only=False):
        ...
        # Background stage : It generates background image & scales it to the desired size..
        ...
        # Parent stage : It stores parent image, parent mask & parent foreground..
        ...
        # Child stage : stores child image, child foreground, child mask..
...
## This discriminator network is designed to take an input image and output two sets of logits: one for classifying the image & the other for determining the authenticity of the image (real or fake).
...
## This discriminator network is designed for classifying & evaluating parent stage images & masks..
...
## Similar to previous discriminator networks, it consists of an encoder part followed by branches for predicting the fine-grained categories & real/fake authenticity of the input image..
...
## Defined for encoding input images into latent codes for different components..
...
## The final layer of each submodel produces the latent code for the respective component..
...
################################## BI_DIS #######################################
## This module adds Gaussian noise to the input tensor during the forward pass.
