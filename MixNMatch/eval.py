...
## Updated file for the desired output images dimentions.. 
...
def save_img(img, file, target_size=(360,640)):  
    img = img.cpu() 
...
def eval_code():
    ...
    with torch.no_grad():
        ...
        fake_imgs, _, _, _ = netG(fake_z2, fake_c, fake_p, 'code' )
        img = fake_imgs[2]
         
    save_img(img, OUT, target_size=(360, 640))
...
def eval_feature():
    ...
    with torch.no_grad():
        ...
        fake_imgs, _, _, _ = netG(None, fake_c, shape_feature, 'feature' )
        img = fake_imgs[2]       
